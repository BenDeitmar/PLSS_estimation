...
if __name__ == "__main__":
	...
	try:
		...
		AvgTimes_LedoitWolf = np.load(data_path+'Fig12_AvgTimes_LedoitWolf_Ex{}.npy'.format(ExampleNumber))
		LedoitWolf_EstimatedEV_List = []
		AllDataMatrices = []
		AllPopEVs = []
		AllChosenDims = []
		for n in n_List:
			AllDataMatrices.append(np.load(data_path+'Fig12_DataMatrices_n={}_Ex{}.npy'.format(int(n),ExampleNumber)))
			AllPopEVs.append(np.load(data_path+'Fig12_PopEVs_n={}_Ex{}.npy'.format(int(n),ExampleNumber)))
			AllChosenDims.append(np.load(data_path+'Fig12_DimsChosen_n={}_Ex{}.npy'.format(int(n),ExampleNumber)))
			LedoitWolf_EstimatedEV_List.append(np.load(data_path+'Fig12_LedoitWolf_Estimators_n={}_Ex{}.npy'.format(int(n),ExampleNumber)))
			_,NN = LedoitWolf_EstimatedEV_List[0].shape
	except:
		print('#############################')
		print("Error: could not load the results of the Ledoit-Wolf estimator")
		print("try running Figure_12_1_Preparation.R first")
		print('#############################')
		assert 0==1
	...
	for key in ToCompare:
		np.save(data_path+'Fig12_AvgTimes_vec{}{}_{}_Ex{}'.format(SampVecNr,PopVecNr,key,ExampleNumber),np.array(AvgTimes[key]))
		np.save(data_path+'Fig12_AvgErrors_vec{}{}_{}_Ex{}'.format(SampVecNr,PopVecNr,key,ExampleNumber),np.array(AvgErrors[key]))
		np.save(data_path+'Fig12_Variances_vec{}{}_{}_Ex{}'.format(SampVecNr,PopVecNr,key,ExampleNumber),np.array(Variances[key]))
		np.save(data_path+'Fig12_AllErrors_vec{}{}_{}_Ex{}'.format(SampVecNr,PopVecNr,key,ExampleNumber),np.array(AllErrors[key]))
	np.save(data_path+'Fig12_RejectedIndices_vec{}{}_Ex{}'.format(SampVecNr,PopVecNr,ExampleNumber),RejectedIndices)
