...
d_List = seq(50,750,by=50)
output_file_path = paste(dirname(rstudioapi::getSourceEditorContext()$path),"data","Fig9_d_List.npy",sep='/')
...
for (d in d_List){
  ...
  filename <- paste0("Fig9_LedoitWolf_Estimators_d=", d, "_c=", c, "_Ex", ExampleNumber, ".npy")
  ...
  if (ExampleNumber==3){
    ...
    filename <- paste0("Fig9_DimsChosen_d=", d, "_c=", c, "_Ex", ExampleNumber, ".npy")
    output_file_path = paste(dirname(rstudioapi::getSourceEditorContext()$path),"data",filename,sep='/')
    npySave(output_file_path, D)
  }
  
  filename <- paste0("Fig9_DataMatrices_c=", c, "_Ex", ExampleNumber, "_d=", d, ".npy")
  output_file_path = paste(dirname(rstudioapi::getSourceEditorContext()$path),"data",filename,sep='/')
  AllDataMatrices <- np$array(AllDataMatrices)
  np$save(output_file_path, AllDataMatrices)
  
  filename <- paste0("Fig9_PopEVs_c=", c, "_Ex", ExampleNumber, "_d=", d, ".npy")
  output_file_path = paste(dirname(rstudioapi::getSourceEditorContext()$path),"data",filename,sep='/')
  AllPopEVs <- np$array(AllPopEVs)
  np$save(output_file_path, AllPopEVs)
  
}
filename <- paste0("Fig9_AvgTimes_LedoitWolf_c=", c, "_Ex", ExampleNumber, ".npy")
output_file_path = paste(dirname(rstudioapi::getSourceEditorContext()$path),"data",filename,sep='/')
