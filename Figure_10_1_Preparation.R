...
output_file_path = paste(dirname(rstudioapi::getSourceEditorContext()$path),"data","Fig10_d_List.npy",sep='/')
npySave(output_file_path, d_List)

NN = 50

##Figure 10a
c = 1/5 ## for memory reasons, this is 1/5 instead of 1/10
ExampleNumber=1
...
for (d in d_List){
  ...
  filename <- paste0("Fig10_LedoitWolf_Estimators_d=", d, "_c=", c, "_Ex", ExampleNumber, ".npy")
  output_file_path = paste(dirname(rstudioapi::getSourceEditorContext()$path),"data",filename,sep='/')
  npySave(output_file_path, A)
  
  filename <- paste0("Fig10_DataMatrices_c=", c, "_Ex", ExampleNumber, "_d=", d, ".npy")
  output_file_path = paste(dirname(rstudioapi::getSourceEditorContext()$path),"data",filename,sep='/')
  AllDataMatrices <- np$array(AllDataMatrices)
  np$save(output_file_path, AllDataMatrices)
  
  filename <- paste0("Fig10_PopEVs_c=", c, "_Ex", ExampleNumber, "_d=", d, ".npy")
  output_file_path = paste(dirname(rstudioapi::getSourceEditorContext()$path),"data",filename,sep='/')
  AllPopEVs <- np$array(AllPopEVs)
  np$save(output_file_path, AllPopEVs)
  
}
filename <- paste0("Fig10_AvgTimes_LedoitWolf_c=", c, "_Ex", ExampleNumber, ".npy")
output_file_path = paste(dirname(rstudioapi::getSourceEditorContext()$path),"data",filename,sep='/')
