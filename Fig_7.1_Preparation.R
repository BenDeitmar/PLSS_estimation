...
#source("example_data_in_R.R", local = FALSE)
source("example_data_in_R.R")
...
for (d in d_List){
  ...
  filename <- paste0("Fig7_LedoitWolf_Estimators_d=", d, "_c=", c, "_Ex", ExampleNumber, ".npy")
  output_file_path = paste(dirname(rstudioapi::getSourceEditorContext()$path),"data",filename,sep='/')
  npySave(output_file_path, A)
  
  filename <- paste0("Fig7_DataMatrices_c=", c, "_Ex", ExampleNumber, "_d=", d, ".npy")
  output_file_path = paste(dirname(rstudioapi::getSourceEditorContext()$path),"data",filename,sep='/')
  AllDataMatrices <- np$array(AllDataMatrices)
  np$save(output_file_path, AllDataMatrices)
  
  filename <- paste0("Fig7_PopEVs_c=", c, "_Ex", ExampleNumber, "_d=", d, ".npy")
  output_file_path = paste(dirname(rstudioapi::getSourceEditorContext()$path),"data",filename,sep='/')
  AllPopEVs <- np$array(AllPopEVs)
  np$save(output_file_path, AllPopEVs)
  
}
filename <- paste0("Fig7_AvgTimes_LedoitWolf_c=", c, "_Ex", ExampleNumber, ".npy")
output_file_path = paste(dirname(rstudioapi::getSourceEditorContext()$path),"data",filename,sep='/')
npySave(output_file_path, AvgTimeList)
output_file_path = paste(dirname(rstudioapi::getSourceEditorContext()$path),"data","Fig7_d_List.npy",sep='/')
