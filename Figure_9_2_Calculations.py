...
from VisualizationTools import PopEV_Y_maker, H_Estimation
...
if __name__ == "__main__":
    ...
    if 1: #Figure 9a
        ...
        data_name='Stieltjes_a'
        resolution=1/100
        ToCompare = {'LedoitWolf', 'ElKaroui','MPI'}
    if 0: #Figure 9b
        z = -0.5+0.1j
        g = lambda lam: 1/(lam-z)
        data_name='Stieltjes_b'
        resolution=1/100
        ToCompare = {'LedoitWolf', 'ElKaroui'}
    if 0: #Figure 9c
        z = 0+0.1j
        g = lambda lam: 1/(lam-z)
        data_name='Stieltjes_c'
        resolution=1/100
        ToCompare = {'LedoitWolf', 'ElKaroui'}
    ###################
    ...
    try:
        d_List = np.load(data_path+'Fig9_d_List.npy')
        AvgTimes_LedoitWolf = np.load(data_path+'Fig9_AvgTimes_LedoitWolf_c={}_Ex{}.npy'.format(c,ExampleNumber))
        LedoitWolf_EstimatedEV_List = []
        AllDataMatrices = []
        AllPopEVs = []
        ChosenDimList = []
        for d in d_List:
            AllDataMatrices.append(np.load(data_path+'Fig9_DataMatrices_c={}_Ex{}_d={}.npy'.format(c,ExampleNumber,int(d))))
            AllPopEVs.append(np.load(data_path+'Fig9_PopEVs_c={}_Ex{}_d={}.npy'.format(c,ExampleNumber,int(d))))
            LedoitWolf_EstimatedEV_List.append(np.load(data_path+'Fig9_LedoitWolf_Estimators_d={}_c={}_Ex{}.npy'.format(int(d),c,ExampleNumber)))
            _,NN = LedoitWolf_EstimatedEV_List[0].shape
    except:
        print('#############################')
        print("Error: could not load the results of the Ledoit-Wolf estimator")
        print("try running Figure_9_1_Preparation.R with the same choices for c and ExampleNumber first")
        print('#############################')
        assert 0==1
    ...
    for key in ToCompare:
        np.save(data_path+'Fig9_AvgTimes_{}_c={}_{}_Ex{}'.format(key,c,data_name,ExampleNumber),np.array(AvgTimes[key]))
        np.save(data_path+'Fig9_AllErrors_{}_c={}_{}_Ex{}'.format(key,c,data_name,ExampleNumber),np.array(AllErrors[key]))
